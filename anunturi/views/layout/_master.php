		...
		<div class="main container padding2">
			<div class="navbar">
				<div class="navbar-inner">
					...
					<ul class="nav">
						<li class="<?php if($active_page == 'index') echo 'active'?>"><a
							href="<?php echo base_url()?>advert/index">Acasa</a>
						</li>
						...
						<li class="<?php if($active_page == 'about') echo 'active'?>"><a
							href="<?php echo base_url()?>advert/about">Despre</a>
						</li>
						<div class="btn-group">
							<a class="btn dropdown-toggle" data-toggle="dropdown" href="#"> <?=$username?>
								<span class="caret"></span>
							</a>
							<ul class="dropdown-menu">
								<?php if($is_logged):?>
								<li><a tabindex="-1" href="<?=base_url().'user/logout'?>">Deconectare</a>
								</li>
								<?php else:?>
								<li><a tabindex="-1" href="<?=base_url().'user/login'?>">Autentificare</a>
								</li>
								<li><a tabindex="-1" href="<?=base_url().'user/newUser'?>">Inregistrare</a>
								</li>
								<?php endif;?>
							</ul>
						</div>
					...
					<form class="navbar-search pull-right" method="post"
						action="<?php echo base_url();?>advert/search">
						<input id="search_bar" name="kwd" type="text" class="search-query"
							placeholder="Search" value="<?php if(!empty($kwd)) echo $kwd; ?>">
					</form>
				</div>
			</div>
